...
const add_nota_credito = () => {
  ...
  function functionObtenerEmpresa() {
    ObtenerEmpresaAll().then((result_empresa: any) => {
      if (result_empresa.indicator == 1) {
        ...
        functionGenerarCorrelativo_NC(result_empresa.data[0].Nro_Ruc, 1)
      }
      else {
        Swal.fire({
          title: 'Alerta',
  ...
  function functionClicRadio(value: number) {
    ...
    functionGenerarCorrelativo_NC(dataEmpresa!.Nro_Ruc, value)
  ...
  async function functionEnviarNCredito() {
    let ruta_zip = "";
    ObtenerRutaNCredito(nroNotaCredito, dataEmpresa!.Nro_Ruc).then((result_2: any) => {
  ...
  function FunctionRegistrarNCredito() {
    ...
    RegistrarNCredito(modal_ncredito).then((result: any) => {
      if (result.indicator == 1) {
        ...
        functionGenerarCorrelativo_NC(dataEmpresa!.Nro_Ruc, Id_Tipo_Documento)
  ...
  function FunctionModificarEstadoDocumento() {
    ModificarEstadoDocumento(dataEmpresa!.Nro_Ruc, numeroFE, 0).then((result: any) => {
      if (result.indicator == 1) {
        setTimeout(function () {
          functionArmarPDF(numeroFE, Id_Cliente);
          functionListarDocumentos(CurrentPage, numeroDocumento, Id_Tipo_Documento);
        }, 2000);
        toast.success(
          `${result.message}`, {
          duration: 2000,
        ...
        <CardBody className="rounded-md pt-2 px-4">
          <div className="justify-between flex">
            ...
            <Chip color="teal" variant="ghost"
              value={`N° Nota Crédito : ${nroNotaCredito}`}
...
export default add_nota_credito